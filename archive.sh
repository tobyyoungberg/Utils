...
xcodebuild -create-xcframework -framework build/ios.xcarchive/Products/Library/Frameworks/Utils.framework -framework build/iossimulator.xcarchive/Products/Library/Frameworks/Utils.framework -output build/Utils.xcframework

# Get rid of the intermediary xcarchive files
rm -rf build/*.xcarchive

zip -r Utils.xcarchive.zip build
